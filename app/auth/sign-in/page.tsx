...
export default function SignInForm() {
  ...
  return (
    <form action={formAction}
      ...
      {isPending ?
      <button type='button'
        className="flex w-sm justify-center rounded-md bg-indigo-300 px-3 py-1.5 text-sm/6 font-semibold text-white hover:cursor-not-allowed">
        Signing in
      </button>
      :
      <button type="submit"
      ...
      <p>don{`'`}t have an account? <span className='text-blue-800'><Link href={`/auth/sign-up`}>sign up</Link></span></p>
    </form>
  );
}